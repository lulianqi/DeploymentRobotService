...
namespace DeploymentRobotService.Controllers
{
    ...
    {
        private readonly ILogger<WxOauthController> _logger;

        public WxOauthController(ILogger<WxOauthController> logger)
        {
            _logger = logger;
        }
        ...
        public async Task<ActionResult<string>> Get()
        {
            ...
            string userId = await ApplicationRobot.WxConnector.NowWxHelper.GetUserInfoByCodeAsync(code);
            if (string.IsNullOrEmpty(userId))
            {
                _logger.LogError("GetUserInfoByCodeAsync fail");
                return StatusCode(500, "GetUserInfoByCodeAsync fail");
            }
