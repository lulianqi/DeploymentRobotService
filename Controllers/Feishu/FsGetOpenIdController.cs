...
namespace DeploymentRobotService.Controllers
{
    ...
    [ApiController]
    public class FsGetOpenIdController : ControllerBase
    {
        private readonly ILogger<FsGetOpenIdController> _logger;


        public FsGetOpenIdController(ILogger<FsGetOpenIdController> logger)
        {
            _logger = logger;
        }

        [HttpGet]
        public async Task<string> GetAsync([Required] string mail)
        {
            return (await ApplicationRobot.FsConnector?.NowFsHelper.GetOpenIdByMailOrMobileAsync(mail))??"";
        }
        ...
        [Route("All")]
        [HttpGet]
        public  Dictionary<string, FsUserInfo> GetAllAsync()
        {
            return ApplicationRobot.FsRobotBusinessData.FsUserDc;
        }
    }
}
